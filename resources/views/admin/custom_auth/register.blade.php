...
<div class="log-w3">
    ...
    font-weight: bold;">'.$message.'</span>';
        ...
        ?>
        <form action="{{\Illuminate\Support\Facades\URL::to('/register')}}" method="post">
            {{csrf_field()}}
            <input type="text" class="ggg" name="admin_name" value="{{old('admin_name')}}" placeholder="Name" required="">
            <input type="email" class="ggg" name="admin_email" value="{{old('admin_email')}}"placeholder="Email" required="">
            <input type="text" class="ggg" name="admin_phone" value="{{old('admin_phone')}}"placeholder="Phone" required="">
            ...
            <div style="font-size: 20px;margin-top: 10px;font-weight: bold"><a href="{{\Illuminate\Support\Facades\URL::to('/admin')}}">Đăng nhập</a></div>
